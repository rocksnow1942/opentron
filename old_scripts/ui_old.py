...
def build_operation_para():
    ...
    try:
        ...
        pick = 1
    deckplan_list = ['default','default-5ml','default-mag']
    operation_para = {}
...
def print_deck_plans():
    ...
    print("2).default-5ml : 1/3:elisa strip; 2:trough; *4:5ml-50ml rack*; 5:ep rack; 9:trash")
    print('3).default-mag : *1:magnet*; 2:trough; 3:elisa strip; 4:15-50ml rack; 5:ep rack; 9:trash')
...
def save_plan(operation_para):
    ...
    if input('Review the parameters. Ready to save? y/n : ') == 'y':
        name = os.path.join(save_path,(input('Name the protocol : ')+'.json'))
...
def choose_saved_protocol():
    ...
    """
    protocol_list = glob.glob(os.path.join(save_path,'*.json'))
    print('List of saved protocols:')
    for i,name in enumerate(protocol_list):
        print('{}). {}'.format(i+1, name[save_path_len:-5]))
    choice = input('Which protocol to run? (1,2,3) : ')
    file_path = protocol_list[int(choice)-1]
    return file_path
...
def read_saved_protocol(file_path):
    ...
    with open(file_path,'rt') as file:
        para = json.load(file)
    print('+'*50)
    print("Type in parameters for the procotl:")
